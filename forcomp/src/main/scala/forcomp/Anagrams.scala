...
object Anagrams extends AnagramsInterface:
   ...
   */
  def choosek(occurr: Occurrences, k: Int): List[Occurrences] =
      val l = occurr.length
      if k == 0 then List(Nil)
      else
        // for {
        //   ii <- (0 until l).toList
        //   jj <- List(occurr.drop(ii).head._1)
        //   kk <- (1 to occurr.drop(ii).head._2)
        //   ll <- choosek(occurr.drop(ii + 1), k - 1)
        // } println(s"($jj, $kk) :: $ll")
        for
          ii <- (0 until l).toList
          jj <- List(occurr.drop(ii).head._1)
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = 
    ...
    def update(z: (Char, Int)): (Char, Int) = 
      val (c, n) = z
      (c, n - yMap.apply(c))

    x.map(update(_)).filter(_._2 > 0)
    ...
    def occurrenceAnagrams(occurrences: Occurrences): List[Sentence] = 
      if occurrences.isEmpty then List(Nil)
        ...
        for
          ...
          rest <- occurrenceAnagrams(subtract(occurrences, wordOccurrences(word)))
        yield word :: rest
