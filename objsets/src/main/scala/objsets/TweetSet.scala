...
class Empty extends TweetSet:
  ...
  def mostRetweetedIter(tweet: Tweet): Tweet = tweet
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  def mostRetweeted: Tweet = 
    mostRetweetedIter(Tweet("dummy", "", -1))

  def mostRetweetedIter(x: Tweet): Tweet = 
    if elem.retweets > x.retweets then left.mostRetweetedIter(right.mostRetweetedIter(elem))
    else left.mostRetweetedIter(right.mostRetweetedIter(x))
